...
pygame.font.init()
FONTE_PONTUACAO = pygame.font.SysFont('verdana', 30)
...
def main(genomas, config): # Fitness Function
    global geracao
    geracao += 1
    if ia_joga:  #Criando vários pássaros
        redes = []
        lista_genomas =[]
        pardals = []
    ...
    while rodando:
        relogio.tick(60) # Fps do jogo para jogar com a ia ,
        ...
        indice_cano = 0
        if len(pardals) > 0:
            if len(canos) > 1 and pardals[0].x > (canos[0].x + canos[0].CANO_TOPO.get_width()):
                indice_cano = 1
        else:
            rodando = False
            break
        ...
        for i, pardal in enumerate(pardals):
            ...
            output = redes[i].activate((pardal.y, abs(pardal.y - canos[indice_cano].altura),
                                       abs(pardal.y - canos[indice_cano].pos_base)))
            if output[0] > 0.5:
                pardal.pular()
...
if __name__ == '__main__':
    ...
    caminho_config = os.path.join\
        (caminho, r'D:\pythonProject\Flappy Bird\config.txt') # Caminho para o diretório do arquivo config.txt
